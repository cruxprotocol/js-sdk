...
import {StorageHelper} from "../packages"
	...
	describe('after init tests', () => {
		...
		describe("registerCruxID tests", () => {
			...
			it("successful registration", async () => {
				...
				// @ts-ignore
				expect((await new StorageHelper(cruxClient._storage).getJSONAsync('payIDClaim')).virtualAddress).is.to.equal(virtualAddress);
				// @ts-ignore
				expect((await new StorageHelper(cruxClient._storage).getJSONAsync('payIDClaim')).identitySecrets).to.be.a('string');
