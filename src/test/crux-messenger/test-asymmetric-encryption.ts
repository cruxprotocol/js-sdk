...
describe("Asymmetric encryption tests", () => {
    ...
    describe("Asymmetric encryption using eccrypto tests", () => {
        ...
        it("decrypt with wrong key test", async () => {
            ...
            const promise = EncryptionManager.decrypt(encryptedMessage, testKeyManager);
            await expect(promise).to.be.eventually.rejected.with.property('message', 'Decryption failed');
        })
    })
})