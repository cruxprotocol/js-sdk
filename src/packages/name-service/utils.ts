...
const bnsResolveName = async (baseUrl: string, blockstackId: string): Promise<object> => {
    ...
    let nameData;
    try {
        nameData = await cachedFunctionCall(`${options.baseUrl}${blockstackId}`, undefined, httpJSONRequest, [options], async (data) => Boolean(data && data.status && data.status !== "registered_subdomain"));
    } catch (error) {
