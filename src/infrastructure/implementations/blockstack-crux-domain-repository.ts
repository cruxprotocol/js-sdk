...
export class BlockstackCruxDomainRepository implements ICruxDomainRepository {
    ...
    public get = async (domain: string): Promise<CruxDomain|undefined> => {
        ...
        return new CruxDomain(domain, domainRegistrationStatus, domainClientConfig);
    }
    public save = async (cruxDomain: CruxDomain, configKeyManager: IKeyManager): Promise<CruxDomain> => {
        await BlockstackService.putClientConfig(cruxDomain.domain, cruxDomain.config, this._bnsNodes, configKeyManager);
        return cruxDomain;
    }
    public restore = async (keyManager: IKeyManager, domainContext?: string): Promise<CruxDomain|undefined> => {
        const associatedDomain = await BlockstackService.restoreDomain(keyManager, this._bnsNodes, domainContext);
        if (!associatedDomain) {
            return;
        }
        const domainClientConfig = await BlockstackService.getClientConfig(associatedDomain, this._bnsNodes);
        return new CruxDomain(associatedDomain, DomainRegistrationStatus.REGISTERED, domainClientConfig);
    }
}
